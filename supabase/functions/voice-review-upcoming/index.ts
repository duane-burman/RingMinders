...
serve(async (req: Request) => {
  ...
  if (reminderIdsRaw) {
    ...
    if (digitIndex >= 0 && digitIndex < reminderIds.length) {
      ...
      const actionUrl = (`${BASE_URL}/voice-reminder-action?${sessionParams}&reminderId=${selectedId}&scheduledAt=${encodeURIComponent(reminder.scheduled_at)}&returnTo=upcoming`).replace(/&/g, '&amp;')
      LOG('return-4')
...
<Response>
  <Gather input="dtmf" numDigits="1" finishOnKey="" action="${actionUrl}" method="POST" timeout="10">
    ...
    <Say voice="Google.en-US-Neural2-D">Press pound to return to your reminders. Press star then pound to cancel this reminder.</Say>
  </Gather>
  <Hangup/>
</Response>`)
    }
