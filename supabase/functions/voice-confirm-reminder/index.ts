...
<Response>
  <Say voice="Google.en-US-Neural2-D">Your reminder has been saved. You will receive a call at ${spokenCallback} on ${spoken}. Press 1 to create another reminder, or hang up to end the call. Thank you for using the Reminder Service.</Say>
...
<Response>
  <Gather input="dtmf" numDigits="1" finishOnKey="" action="${BASE_URL}/voice-confirm-reminder?userId=${userId}&amp;userName=${encodeURIComponent(userName)}&amp;scheduledAt=${encodeURIComponent(scheduledAt)}&amp;callbackNumber=${encodeURIComponent(callbackNumber)}&amp;recordingUrl=${encodeURIComponent(recordingUrl)}&amp;recordingDuration=${recordingDuration}" method="POST" timeout="15">
    <Say voice="Google.en-US-Neural2-D">Your reminder is set for ${spoken}. Here is your message:</Say>
    <Play>${recordingUrl}</Play>
    <Say voice="Google.en-US-Neural2-D">Press pound to confirm and save this reminder. Press star to start over.</Say>
  </Gather>
  <Hangup/>
</Response>`)
})
