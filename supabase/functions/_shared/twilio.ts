...
<Response>
  <Say voice="Google.en-US-Studio-M">${message}</Say>
  <Hangup/>
</Response>`
}
...
<Response>
  <Gather input="dtmf" action="${xmlAttr(opts.action)}" method="POST" ${numDigits} ${finishOnKey} ${timeout}>
    <Say voice="Google.en-US-Studio-M">${opts.message}</Say>
  </Gather>
  <Say voice="Google.en-US-Studio-M">We did not receive any input. Goodbye.</Say>
  <Hangup/>
</Response>`
}
