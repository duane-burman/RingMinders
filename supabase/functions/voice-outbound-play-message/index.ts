...
<Response>
  <Gather input="dtmf" numDigits="1" timeout="15" action="${BASE_URL}/voice-outbound-play-message?reminder_id=${reminderId}&amp;played=true" method="POST">
    <Say voice="Google.en-US-Studio-M">Here is your reminder:</Say>
    <Play>${playUrl6}</Play>
    <Say voice="Google.en-US-Studio-M">Press 1 to repeat this message. Press 2 to hear the message one more time and then end the call. Or simply hang up when finished.</Say>
  </Gather>
  <Hangup/>
</Response>`)
})
